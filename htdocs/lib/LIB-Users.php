...
class Users extends Core {
  ...
  function verify ($email, $password) {
    ...
    if (!is_array($user)) {
      $this->error = "Ungültiger Benutzer oder Passwort.";
      return false;
    }

    // (I2) PENDING ACTIVATION
    if ($user["hash_code"]!=null) {
      $this->error = "Bitte aktivieren Sie zuerst Ihr Konto.";
      return false;
    }
  ...
  function register ($name, $email, $password) {
    ...
    if (is_array($this->get($email))) {
      $this->error = "$email ist bereits registriert.";
      return false;
    }
  ...
  function hsend ($id) {
    ...
    $user = $this->get($id, "A");
    if (!is_array($user)) {
      $this->error = "Ungültiger Benutzer";
      return false;
    }
  ...
  function hactivate ($i, $h) {
    ...
    if ($user["hash_time"]==null) {
      $this->error = "Konto bereits aktiv";
      return false;
    }

    // (N3) HASH CHECK
    if (strtotime("now") >= strtotime($user["hash_time"]) + $this->hvalid) {
      $this->error = "Aktivierungslink abgelaufen";
      return false;
    }
    if ($user["hash_code"]!=$h) {
      $this->error = "Ungültiger Aktivierungslink";
      return false;
    }
