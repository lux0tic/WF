...
class Report extends Core {
  ...
  function supitems ($id) {
    ...
    while ($r = $this->DB->stmt->fetch()) {
      fputcsv($f, [$r["item_sku"], $r["sup_sku"], $r["item_name"], $r["item_desc"], $r["item_unit"], $r["unit_price"], $r[""], 
        $r["item_eingangdate"], $r["item_eingangqty"], $r["item_fake"], $r["item_order"], $r["item_ruckstand"], $r["item_eknetto"], $r["item_ekgesamt"], $r["item_vpe"], $r["item_gewichst"], $r["item_zoll"], $r["item_herkunft"], $r["item_ean"], $r["item_lagerplatz"]]);
    }
    fclose($f);
  }
